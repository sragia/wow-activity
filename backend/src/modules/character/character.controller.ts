...
export class CharacterController {
  ...
  async addProfile(@Body() payload: AddCharacterPayload, @Req() request: Request): Promise<Character> {
    const character = await this.bnetService.getCharacter(payload.name, payload.realm, payload.region);

    if (!character) {
      throw new BadRequestException(
        'Couldnt find character'
      )
    }
